...
namespace DAL
{
    public class SchoolYear_DAL
    {
        ...
        public static SchoolYear_DTO LoadASY(string ID)
        {
            string QueryString = string.Format("Select * from NAMHOC where MaNamHoc='{0}' ", ID);
            DataTable dataTable = DataProvider.dataTableQuery(QueryString);
            if (dataTable!=null && dataTable.Rows.Count > 0)
            {
                SchoolYear_DTO schoolYear_DTO = new SchoolYear_DTO();
                schoolYear_DTO.ID = dataTable.Rows[0]["MaNamHoc"].ToString();
                schoolYear_DTO.Begin =Convert.ToInt32(dataTable.Rows[0]["TuNam"].ToString());
                schoolYear_DTO.End = Convert.ToInt32(dataTable.Rows[0]["DenNam"].ToString());
                return schoolYear_DTO;
            }
            return null;
        }
    }
