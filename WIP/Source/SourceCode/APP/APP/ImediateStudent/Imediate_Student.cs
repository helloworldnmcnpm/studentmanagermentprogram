...
{
    public partial class Imediate_Student : MetroFramework.Forms.MetroForm
    {
        ...
        public Imediate_Student()
        {
            InitializeComponent();

        }

        private void Imediate_Student_Load(object sender, EventArgs e)
        {
            int count = 0;
            int ma = 0, mi = 0;
            string Sma = "", Smi = "", Current = "";
            RateLabel.Text = "Tỷ lệ học sinh khá toàn trường: " + percent + "%";
            ...
            {
                if (process_DTOs[i].TotalScore >= 6.5&&process_DTOs[i].TotalScore<8)
                    ListImediate.Add(process_DTOs[i]);
            }
            DataRow dataRow;
            number = ListImediate.Count;
            NumberLabel.Text = "Tổng số học sinh khá: " + number;
            ...
            {
                count = 0;
                for (int j = 0; j < ListImediate.Count; j++)
                {
                    if (ListImediate[j].ClassID == Class_BUL.LoadBySC(SchoolYearID)[i].ID)
                    {
                        count++;
            ...
            Max.Text = "Lớp có tổng số học sinh khá nhiều nhất (" + ma + " Học sinh): " + Current;
            Current = "";
            count = 0;
            mi = ListImediate.Count;
            ...
            Min.Text = "Lớp có tổng số học sinh khá ít nhất (" + mi + " Học sinh): " + Current;
        }
        ...
        {
            ImediateStudent.OrderByClass orderByClass = new ImediateStudent.OrderByClass();
